...
export default function FormDoc(toast) {
 ...
 que contendrán la información de los campos del formulario*/
  const agregar = () => {
    ...
    if (!nombre || !correo || !telefono) {
      ...
      toastRef.current.show("Debe ingresar una foto al registro");
      ...
      subirImagenesStorage().then((resp) => {
        ...
        db.collection("docentes")
          ...
          .then(() => {
            //si todo es correcto
            navegacion.navigate("docente");
          })
          .catch(() => {
            //si no es posible almacenar
            toastRef.current.show("No es posible registrar al docente");
          });
      });
    }
  };
  ...
  const subirImagenesStorage = async () => {
    ...
    el nombre de cada imagen será generado por el random de uuid*/
        const ref = firebase.storage().ref("docentes").child(uuid());
    ...
    insertar la sucursal*/
          await firebase
            .storage()
            .ref(`docentes/${resultado.metadata.name}`)
            .getDownloadURL()
            .then((urlFoto) => {
...
function Formulario(propiedades) {
  const { setNombre, setCorreo, setTelefono } = propiedades;
